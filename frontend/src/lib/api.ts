...
import { Station, Reading, PredictionInput, PredictionResult, ExplanationResult, SimulationInput, SimulationResult, ModelMetrics, ModelInfo, AnnualProjection } from './types';
...
export async function getHistorical(
  stationId?: string,
  options?: { year?: number; month?: number; days?: number; fields?: string; resample?: string }
): Promise<Reading[]> {
  ...
  if (options?.resample) params.set('resample', options.resample);
...
// ---------------------------------------------------------------------------
// Annual Projections (2019–2029)
// ---------------------------------------------------------------------------
export async function getAnnualProjections(stationId?: string, season?: string): Promise<AnnualProjection[]> {
  const params = new URLSearchParams();
  if (stationId) params.set('station_id', stationId);
  if (season && season !== 'all') params.set('season', season);
  const qs = params.toString();
  return apiFetch<AnnualProjection[]>(qs ? `/projections/annual?${qs}` : '/projections/annual');
}
